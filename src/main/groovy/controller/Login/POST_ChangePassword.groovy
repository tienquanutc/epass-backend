...
class POST_ChangePassword extends VertxController<AppConfig> {
    ...
    void validate(RoutingContext context, HttpServerRequest request) {
        ...
        Password.Validate.isMD5Hash(json.old_password as String, "old password must be a md5 hash")
        Password.Validate.isMD5Hash(json.new_password as String, "new password must be a md5 hash")
